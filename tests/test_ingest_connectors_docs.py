...
EXPECTED_COMMANDS = (
    ...
    r"node packages\cli\tests\ingest-connector-mcp-api-replay.test.mjs",
    ...
    r"node apps\web\tests\ingest-connector-mcp-fixture-state.test.mjs",
    ...
    r"node packages\schemas\tests\ingest-connector-mcp-api.test.mjs",
    ...
    r"node apps\api\tests\ingest-connector-mcp-schema-alignment.test.mjs",
    r"node packages\cli\tests\ingest-connector-mcp-api-replay.test.mjs",
...
class IngestConnectorDocsTests(unittest.TestCase):
    ...
    def test_document_has_required_sections_files_symbols_and_commands(self) -> None:
        ...
        for file_path in EXPECTED_PLANNED_SCHEMA_FILES:
            with self.subTest(file_path=file_path):
                self.assertIn(f"`{file_path}`", self.text)
    ...
    def test_document_states_local_only_no_network_and_default_untrusted(self) -> None:
        phrases = (
            ...
            "sdk fixture harnesses",
            "cli replay",
            "web fixture state",
            "openapi/fixture parity",
            "mcp api e2e parity",
        )
