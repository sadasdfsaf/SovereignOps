...
import {
  ...
  type WorkspaceSessionSnapshotRetentionCleanupInventoryRoutesOptions,
...
export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayCliResult {
  readonly exitCode: number;
  readonly stdout: string;
  readonly stderr: string;
}

export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRunOptions {
  readonly cwd?: string;
  readonly dispatch?: WorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher;
}

export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRequest {
...
export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayResponse {
  readonly status: number;
  readonly headers: Readonly<Record<string, string>>;
  readonly body: unknown;
}

export type WorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher = (
  request: WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRequest,
) =>
  | WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayResponse
  | Promise<WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayResponse>;
...
export const WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION =
  "workspace-session-snapshot-retention-cleanup-inventory-api-requests/v1";
const LEGACY_WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION =
  "workspace-session-snapshot-retention-cleanup-inventory-api-requests.v1";

export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureBundle {
  readonly schemaVersion:
    typeof WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION;
  readonly generatedAt: string;
  readonly apiBase?: string;
  readonly fixtureRefs: readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRef[];
  readonly requests: readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRequest[];
}

export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRef {
  readonly id: string;
  readonly fixturePath: string;
}

export interface WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRequest {
  readonly id: string;
...
interface ReplayedRequest {
  readonly fixture: WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRequest;
...
const PREVIEW_ROUTE =
  `${DEFAULT_WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_ROUTE_BASE_PATH}/preview`;
const DEFAULT_WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_FIXTURE =
...
const HELP_TEXT = {
  ...
  options: {
    fixture:
      `Local workspace/session snapshot retention cleanup inventory API request fixture bundle JSON path, for example ${DEFAULT_WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_FIXTURE}.`,
    method: "Optional HTTP method filter, for example POST.",
    route: `Optional exact route path filter, for example ${PREVIEW_ROUTE}.`,
    id:
      "Optional exact request id filter, for example api_workspace_session_snapshot_retention_cleanup_inventory_preview.",
  },
};
...
const RAW_LOCK_TOKEN_PATTERNS = [
  /\block_[A-Za-z0-9_-]{4,}\b/g,
  /\block-token-[A-Za-z0-9_-]{4,}\b/gi,
  /\blockToken\s*[:=]\s*["']?[^"',;\s]+["']?/gi,
];
...
export async function runWorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayCli(
  argv: readonly string[] = [],
  options: WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRunOptions = {},
): Promise<WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayCliResult | undefined> {
  const parsed = parseArgv(argv);
  if (!isWorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayParsedCommand(parsed)) {
    return undefined;
  }
  ...
  if (hasHelp(parsed)) {
    return jsonSuccess({
      kind: "workspace-session-snapshot-retention-cleanup-inventory-api-replay.help",
      ...HELP_TEXT,
    });
  }

  const commandLength = workspaceSessionSnapshotRetentionCleanupInventoryApiReplayCommandLength(
    parsed.positionals,
  );
  ...
  try {
    ...
    const replayed = await replayRequests(
      requests,
      fixtureRefs,
      options.dispatch ?? createWorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher(),
    );
    ...
    if (error instanceof WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError) {
      return jsonFailure(error.exitCode, error.code, error.message, error.details);
    }

    return jsonFailure(
      1,
      "workspace_session_snapshot_retention_cleanup_inventory_api_replay_error",
      error instanceof Error ? error.message : String(error),
    );
  }
}

export function isWorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayCommand(
  argv: readonly string[],
): boolean {
  return isWorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayParsedCommand(parseArgv(argv));
}

export async function loadWorkspaceSessionSnapshotRetentionCleanupInventoryApiRequests(
  fixturePath: string,
  options: Pick<WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRunOptions, "cwd"> = {},
): Promise<readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayRequest[]> {
...
export function createWorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher(
  options: WorkspaceSessionSnapshotRetentionCleanupInventoryRoutesOptions = {},
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher {
  const router = createApiRouter(createWorkspaceSessionSnapshotRetentionCleanupInventoryRoutes(options));
  ...
  return async (request) => {
    try {
      ...
      return apiJsonError(
        500,
        "WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_DISPATCH_ERROR",
        error instanceof Error ? error.message : String(error),
      );
    }
  };
...
async function replayRequests(
  requests: readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRequest[],
  fixtureRefs: ReadonlyMap<string, unknown>,
  dispatch: WorkspaceSessionSnapshotRetentionCleanupInventoryApiDispatcher,
...
function expectationMismatchIssues(
  response: WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayResponse,
...
async function resolveFixturePath(
  ...
  try {
    ...
    if (code === "ENOENT") {
      throw new WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError({
        exitCode: 2,
...
async function loadFixtureRefs(
  refs: readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRef[],
...
function parseFixtureBundle(
  value: unknown,
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureBundle {
  if (!isRecord(value)) {
    throw invalidFixture("fixture root must be a JSON object.");
  }
  if (
    value.schemaVersion !== WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION &&
    value.schemaVersion !==
      LEGACY_WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION
  ) {
    throw invalidFixture(
      `fixture.schemaVersion must be "${WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION}".`,
    );
  }
  ...
  return {
    schemaVersion: WORKSPACE_SESSION_SNAPSHOT_RETENTION_CLEANUP_INVENTORY_API_REQUESTS_SCHEMA_VERSION,
...
function parseFixtureRefs(
  value: unknown,
): readonly WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRef[] {
  if (value === undefined) {
    return [];
  }
  if (!Array.isArray(value)) {
...
function parseFixtureRef(
  value: unknown,
  label: string,
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRef {
...
function parseFixtureRequest(
  value: unknown,
  index: number,
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiFixtureRequest {
...
function isWorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayParsedCommand(
  parsed: ParsedArgv,
): boolean {
  return workspaceSessionSnapshotRetentionCleanupInventoryApiReplayCommandLength(
    parsed.positionals,
  ) > 0;
}

function workspaceSessionSnapshotRetentionCleanupInventoryApiReplayCommandLength(
  ...
  if (
    positionals[0] === "workspace-session-snapshot-retention-cleanup-inventory" &&
    positionals[1] === "api" &&
  ...
  if (
    positionals[0] === "workspace-session-snapshot-retention-cleanup-inventory-api" &&
...
function createRedactor(): Redactor {
  ...
  function redactString(value: string, valuePath: string): string {
    ...
    for (const pattern of RAW_LOCK_TOKEN_PATTERNS) {
      if (!pattern.test(redacted)) {
        pattern.lastIndex = 0;
        continue;
      }
...
function jsonSuccess(
  value: unknown,
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayCliResult {
  return {
    exitCode: 0,
...
function usageError(
  message: string,
): WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError {
  return new WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError({
    exitCode: 2,
    code: "usage_error",
    message,
  });
...
class WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError extends Error {
  readonly exitCode: number;
  ...
  constructor(options: {
    ...
    this.name = "WorkspaceSessionSnapshotRetentionCleanupInventoryApiReplayError";
