...
import {
  createInMemoryCliServices,
  type CliServices,
  type JsonValue,
} from "./commands.ts";

export interface WorkspaceSessionApiReplayCliResult {
  readonly exitCode: number;
  readonly stdout: string;
  readonly stderr: string;
}

export interface WorkspaceSessionApiReplayRunOptions {
  readonly cwd?: string;
  readonly dispatch?: WorkspaceSessionApiDispatcher;
}

export interface WorkspaceSessionApiReplayRequest {
...
export interface WorkspaceSessionApiReplayResponse {
  readonly status: number;
  readonly headers: Readonly<Record<string, string>>;
  readonly body: unknown;
}

export type WorkspaceSessionApiDispatcher = (
  request: WorkspaceSessionApiReplayRequest,
) => WorkspaceSessionApiReplayResponse | Promise<WorkspaceSessionApiReplayResponse>;
...
export const WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION =
  "workspace-session-api-requests/v1";
const LEGACY_WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION =
  "workspace-session-api-requests.v1";

export interface WorkspaceSessionApiFixtureBundle {
  readonly schemaVersion: typeof WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION;
  readonly generatedAt: string;
  readonly apiBase?: string;
  readonly fixtureRefs: readonly WorkspaceSessionApiFixtureRef[];
  readonly requests: readonly WorkspaceSessionApiFixtureRequest[];
}

export interface WorkspaceSessionApiFixtureRef {
  readonly id: string;
  readonly fixturePath: string;
}

export interface WorkspaceSessionApiFixtureRequest {
  readonly id: string;
...
interface ReplayedRequest {
  readonly fixture: WorkspaceSessionApiFixtureRequest;
...
const DEFAULT_WORKSPACE_SESSION_API_FIXTURE =
  "examples/workspace-session/api-requests.json";
const HELP_TEXT = {
  usage: [
    "sovereignops workspace session api replay --fixture <path> [--method <method>] [--route <path>] [--id <id>]",
    "sovereignops workspace-session api replay --fixture <path> [--method <method>] [--route <path>] [--id <id>]",
  ],
  options: {
    fixture: `Local workspace/session API request fixture bundle JSON path, for example ${DEFAULT_WORKSPACE_SESSION_API_FIXTURE}.`,
    method: "Optional HTTP method filter, for example GET or POST.",
    route: "Optional exact route path filter, for example /v1/workspace-session/workspaces.",
    id: "Optional exact request id filter, for example api_workspace_session_list.",
  },
};
...
export async function runWorkspaceSessionApiReplayCli(
  ...
  options: WorkspaceSessionApiReplayRunOptions = {},
): Promise<WorkspaceSessionApiReplayCliResult | undefined> {
  const parsed = parseArgv(argv);
  if (!isWorkspaceSessionApiReplayParsedCommand(parsed)) {
    return undefined;
  }
  ...
  if (hasHelp(parsed)) {
    return jsonSuccess({
      kind: "workspace-session-api-replay.help",
      ...HELP_TEXT,
    });
  }

  const commandLength = workspaceSessionApiReplayCommandLength(parsed.positionals);
  ...
  try {
    ...
    const replayed = await replayRequests(
      requests,
      fixtureRefs,
      options.dispatch ?? createWorkspaceSessionApiDispatcher(),
    );
    const failedRequests = replayed.filter((request) => !replaySucceeded(request)).length;

    return jsonSuccess({
      kind: "workspace-session-api-fixture-replay",
    ...
    if (error instanceof WorkspaceSessionApiReplayError) {
      return jsonFailure(error.exitCode, error.code, error.message, error.details);
    }

    return jsonFailure(
      1,
      "workspace_session_api_replay_error",
      error instanceof Error ? error.message : String(error),
    );
  }
}

export function isWorkspaceSessionApiReplayCommand(argv: readonly string[]): boolean {
  return isWorkspaceSessionApiReplayParsedCommand(parseArgv(argv));
}

export async function loadWorkspaceSessionApiRequests(
  fixturePath: string,
  options: Pick<WorkspaceSessionApiReplayRunOptions, "cwd"> = {},
): Promise<WorkspaceSessionApiFixtureBundle> {
  const fixture = await resolveFixturePath(fixturePath, options.cwd ?? process.cwd());
  return parseFixtureBundle(await readFixtureJson(fixture));
}

export function createWorkspaceSessionApiDispatcher(
  services: CliServices = createInMemoryCliServices({
    auditRecords: [],
    events: [],
    policyRules: [],
    workspaces: [],
...
): WorkspaceSessionApiDispatcher {
  return async (request) => {
    const method = normalizeMethod(request.method);
    const routePath = normalizeRoutePath(request.path);

    try {
      if (method === "GET" && routePath === "/v1/workspaces") {
        return apiJsonResponse(200, {
          kind: "workspace-session.workspaces",
          workspaces: await services.listWorkspaces(),
        });
      }

      if (method === "GET" && routePath === "/v1/workspace-session/workspaces") {
        return apiJsonResponse(200, {
          kind: "workspace-session.workspaces",
          workspaces: await services.listWorkspaces(),
        });
      ...
      if (method === "POST" && routePath === "/v1/workspaces") {
        return apiJsonResponse(201, {
          kind: "workspace-session.workspace",
          workspace: await services.createWorkspace(readWorkspaceCreateBody(request.body)),
        });
      }

      if (method === "POST" && routePath === "/v1/workspace-session/workspaces") {
        return apiJsonResponse(201, {
          kind: "workspace-session.workspace",
          workspace: await services.createWorkspace(readWorkspaceCreateBody(request.body)),
        });
      ...
      if (method === "POST" && routePath === "/v1/workspace-session/summary") {
        return apiJsonResponse(200, workspaceSessionSummaryBody(request.body));
      }

      const workspaceEventMatch = routePath.match(/^\/v1\/workspaces\/([^/]+)\/events$/);
      if (method === "POST" && workspaceEventMatch) {
        return apiJsonResponse(202, {
          kind: "workspace-session.event",
          ...(await services.ingestEvent(
            readIngestEventBody(request.body, decodeURIComponent(workspaceEventMatch[1])),
          )),
        });
      }

      if (method === "POST" && routePath === "/v1/workspace-session/events") {
        return apiJsonResponse(202, {
          kind: "workspace-session.event",
          ...(await services.ingestEvent(readIngestEventBody(request.body))),
        });
      }

      const workspaceEventsMatch = routePath.match(/^\/v1\/workspaces\/([^/]+)\/events$/);
      if (method === "GET" && workspaceEventsMatch) {
        const workspaceId = decodeURIComponent(workspaceEventsMatch[1]);
        const bundle = await services.exportWorkspaceBundle({ workspaceId });
        return apiJsonResponse(200, {
          kind: "workspace-session.events",
          workspaceId,
          events: bundle.events,
        });
      }

      const workspaceBundleMatch = routePath.match(/^\/v1\/workspaces\/([^/]+)\/bundle$/);
      if (method === "GET" && workspaceBundleMatch) {
        return apiJsonResponse(200, {
          kind: "workspace-session.bundle",
          bundle: await services.exportWorkspaceBundle({
            workspaceId: decodeURIComponent(workspaceBundleMatch[1]),
          }),
        });
      }

      if (method === "POST" && routePath === "/v1/workspace-session/export-bundle") {
        const body = requiredRecord(request.body, "body");
        return apiJsonResponse(200, {
          kind: "workspace-session.bundle",
          bundle: await services.exportWorkspaceBundle({
            workspaceId: nonEmptyString(body.workspaceId, "body.workspaceId"),
          }),
        });
      }
      ...
      if (method === "POST" && routePath === "/v1/workspace-session/audit-preview") {
        ...
        return apiJsonResponse(200, {
          ...
          ...(await services.previewAudit({
            workspaceId: optionalNonEmptyString(body.workspaceId, "body.workspaceId"),
            ...optionalFields({
              limit: optionalPositiveInteger(body.limit, "body.limit"),
            }),
          })),
        });
      }

      if (method === "GET" && routePath.match(/^\/v1\/workspaces\/([^/]+)\/audit$/)) {
        const workspaceId = decodeURIComponent(routePath.match(/^\/v1\/workspaces\/([^/]+)\/audit$/)?.[1] ?? "");
        return apiJsonResponse(200, {
          kind: "workspace-session.audit-preview",
          ...(await services.previewAudit({ workspaceId })),
        });
      }
    } catch (error) {
      return apiJsonError(
    ...
    return apiJsonError(
      404,
      "API_ROUTE_NOT_FOUND",
      `No workspace session API route found for ${method} ${routePath}.`,
    );
  };
}

async function replayRequests(
  requests: readonly WorkspaceSessionApiFixtureRequest[],
  fixtureRefs: ReadonlyMap<string, unknown>,
  dispatch: WorkspaceSessionApiDispatcher,
...
async function dispatchSafely(
  dispatch: WorkspaceSessionApiDispatcher,
  request: WorkspaceSessionApiReplayRequest,
): Promise<WorkspaceSessionApiReplayResponse> {
  try {
    return await dispatch(request);
  } catch (error) {
    return apiJsonError(
      500,
      "WORKSPACE_SESSION_API_DISPATCH_ERROR",
      error instanceof Error ? error.message : String(error),
    );
  }
}
...
function expectationMismatchIssues(
  response: WorkspaceSessionApiReplayResponse,
  ...
  const bundle = isRecord(body.bundle) ? body.bundle : {};
  ...
  if (expected.workspaceId !== undefined && workspace.workspaceId !== expected.workspaceId) {
    issues.push("workspaceId");
  }
  if (expected.workspaceCount !== undefined) {
    const workspaces = Array.isArray(body.workspaces) ? body.workspaces : [];
    if (workspaces.length !== expected.workspaceCount) {
      issues.push("workspaceCount");
    }
  }
  if (expected.eventId !== undefined && event.eventId !== expected.eventId) {
    issues.push("eventId");
  }
  if (expected.eventType !== undefined && event.type !== expected.eventType) {
    issues.push("eventType");
  }
  if (expected.eventCount !== undefined) {
    const bodyEvents = Array.isArray(body.events) ? body.events : undefined;
    const bundleEvents = Array.isArray(bundle.events) ? bundle.events : undefined;
    if ((bodyEvents ?? bundleEvents ?? []).length !== expected.eventCount) {
      issues.push("eventCount");
    }
  }
...
async function resolveFixturePath(
  ...
  try {
    ...
    if (code === "ENOENT") {
      throw new WorkspaceSessionApiReplayError({
        exitCode: 2,
...
async function loadFixtureRefs(
  refs: readonly WorkspaceSessionApiFixtureRef[],
...
function parseFixtureBundle(value: unknown): WorkspaceSessionApiFixtureBundle {
  if (!isRecord(value)) {
    throw invalidFixture("fixture root must be a JSON object.");
  }
  if (
    value.schemaVersion !== WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION &&
    value.schemaVersion !== LEGACY_WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION
  ) {
    throw invalidFixture(
      `fixture.schemaVersion must be "${WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION}".`,
    );
  }
  ...
  return {
    schemaVersion: WORKSPACE_SESSION_API_REQUESTS_SCHEMA_VERSION,
...
function parseFixtureRefs(value: unknown): readonly WorkspaceSessionApiFixtureRef[] {
  if (value === undefined) {
    return [];
  }
  if (!Array.isArray(value)) {
...
function parseFixtureRef(value: unknown, label: string): WorkspaceSessionApiFixtureRef {
...
function parseFixtureRequest(value: unknown, index: number): WorkspaceSessionApiFixtureRequest {
  ...
  const expectedChecks = expectedChecksFromResponse(response, value, responseLabel);
...
function expectedChecksFromResponse(
  ...
  if (Object.hasOwn(response, "checks")) {
    ...
    if (!isRecord(checks)) {
      throw invalidFixture(`fixture request ${responseLabel}.checks must be an object.`);
    }
...
function isWorkspaceSessionApiReplayParsedCommand(parsed: ParsedArgv): boolean {
  return workspaceSessionApiReplayCommandLength(parsed.positionals) > 0;
}

function workspaceSessionApiReplayCommandLength(positionals: readonly string[]): number {
  if (
  ...
  if (positionals[0] === "workspace-session-api" && positionals[1] === "replay") {
    return 2;
  }
  return 0;
}
...
function readWorkspaceCreateBody(value: unknown): {
  workspaceId: string;
  name: string;
  deviceId?: string;
  rootKeyRef?: string;
} {
  const body = requiredRecord(value, "body");
  return {
    workspaceId: nonEmptyString(body.workspaceId, "body.workspaceId"),
    name: nonEmptyString(body.name, "body.name"),
    ...(body.deviceId === undefined
      ? {}
      : { deviceId: nonEmptyString(body.deviceId, "body.deviceId") }),
    ...(body.rootKeyRef === undefined
      ? {}
      : { rootKeyRef: nonEmptyString(body.rootKeyRef, "body.rootKeyRef") }),
  };
}

function readIngestEventBody(
  value: unknown,
  ...
  const body = requiredRecord(value, "body");
  const workspaceId = workspaceIdFromRoute ?? nonEmptyString(body.workspaceId, "body.workspaceId");
  const type = nonEmptyString(body.type, "body.type");
  return {
    workspaceId,
    type,
    ...(body.payload === undefined ? {} : { payload: cloneCheckedJson(body.payload, "body.payload") }),
  };
}

function cloneCheckedJson(value: unknown, label: string): JsonValue {
...
function assertJsonValue(value: unknown, label: string): void {
  ...
  if (isRecord(value)) {
    for (const [key, nested] of Object.entries(value)) {
      assertJsonValue(nested, `${label}.${key}`);
    }
    return;
  }
  ...
  throw new WorkspaceSessionApiReplayError({
    ...
    message: `${label} must be JSON-compatible.`,
  });
}
...
function optionalPositiveInteger(value: unknown, label: string): number | undefined {
  ...
  if (!Number.isInteger(value) || value < 1) {
    throw invalidFixture(`${label} must be a positive integer.`);
  }
  return value;
}
...
function workspaceSessionSummaryBody(value: unknown): Record<string, unknown> {
  const body = isRecord(value) ? value : {};
  const workspace = isRecord(body.workspace) ? body.workspace : {};
  const session = isRecord(body.session) ? body.session : {};
  const status = workspaceSessionSummaryStatus(body);

  return {
    kind: "workspace-session.summary",
    workspaceId: stringOrFallback(workspace.id, "wsp_local_workspace"),
    sessionId: stringOrFallback(session.id, "sess_local_workspace"),
    localOnly: true,
    status,
    summaryCards: [
      {
        id: "workspace_session.summary.workspace_open",
        status,
        value: workspace.open === false ? "Closed" : "Open",
        severity: status === "ready" ? "success" : "warning",
      },
      {
        id: "workspace_session.summary.lock_state",
        status,
        value: isRecord(body.lock) && body.lock.locked === true
          ? "Locked to session"
          : "Unlocked",
        severity: status === "ready" ? "success" : "warning",
      },
      {
        id: "workspace_session.summary.approval_gateway",
        status,
        value: isRecord(body.gateway) && body.gateway.status === "ready"
          ? "Gateway ready"
          : "Gateway pending",
        severity: status === "ready" ? "success" : "warning",
      },
      {
        id: "workspace_session.summary.migration_readiness",
        status,
        value: isRecord(body.migration) && body.migration.required === false
          ? "Not required"
          : "Review required",
        severity: status === "ready" ? "success" : "warning",
      },
      {
        id: "workspace_session.summary.backup_readiness",
        status,
        value: isRecord(body.backup) && body.backup.restorable === true
          ? "Restorable"
          : "Backup pending",
        severity: status === "ready" ? "success" : "warning",
      },
...
function workspaceSessionSummaryStatus(body: Record<string, unknown>): "ready" | "attention" {
  const workspace = isRecord(body.workspace) ? body.workspace : {};
  const session = isRecord(body.session) ? body.session : {};
  const gateway = isRecord(body.gateway) ? body.gateway : {};
  const backup = isRecord(body.backup) ? body.backup : {};
...
): WorkspaceSessionApiReplayResponse {
  return {
    status,
...
function routeErrorStatus(error: unknown): number {
  if (error instanceof WorkspaceSessionApiReplayError && error.exitCode === 2) {
    return 400;
  }
...
function routeErrorCode(error: unknown): string {
  ...
  return "workspace_session_request_failed";
}

function routeErrorDetails(error: unknown): Readonly<Record<string, unknown>> | undefined {
  if (error instanceof WorkspaceSessionApiReplayError && error.details !== undefined) {
    return error.details;
  }
  return undefined;
}

function jsonSuccess(value: unknown): WorkspaceSessionApiReplayCliResult {
  return {
    exitCode: 0,
...
): WorkspaceSessionApiReplayCliResult {
  const redactor = createRedactor();
  const errorBody = {
    error: optionalFields({
      code,
...
function usageError(message: string): WorkspaceSessionApiReplayError {
  return new WorkspaceSessionApiReplayError({
    exitCode: 2,
    code: "usage_error",
    message,
  });
...
function invalidFixture(message: string): WorkspaceSessionApiReplayError {
  return new WorkspaceSessionApiReplayError({
    exitCode: 2,
...
class WorkspaceSessionApiReplayError extends Error {
  readonly exitCode: number;
  ...
  constructor(options: {
    ...
    this.name = "WorkspaceSessionApiReplayError";
