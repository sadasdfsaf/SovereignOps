...
import { validateMcpApprovalEvidencePreviewRequestBundle } from "../../schemas/src/mcpApprovalEvidence.ts";
import {
...
function validateFixtureBundleWithSharedSchema(value: unknown): void {
  const rawResult = validateMcpApprovalEvidencePreviewRequestBundle(value);
  const sharedValue = sharedFixtureBundleForValidation(value);
  const sharedResult = sharedValue === value
    ? rawResult
    : validateMcpApprovalEvidencePreviewRequestBundle(sharedValue);
...
function invalidFixture(
  message: string,
  details?: Record<string, unknown>,
): McpApprovalEvidenceReplayError {
  return new McpApprovalEvidenceReplayError({
    exitCode: 2,
